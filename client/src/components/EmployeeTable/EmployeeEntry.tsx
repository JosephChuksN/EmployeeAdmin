...
const EmployeeEntry:React.FC<props> = ({handleShowForm, show, employee, setEmployee, handleEmployeeData, error}) => {
...
const handleClear = ()=>{
  ...
  return (
    <form onSubmit={handleSubmit}  className={` flex pt-10  flex-col transition-all delay-150 duration-300 ease-in-out w-full ${show ?  "opacity-100  visible" : " opacity-0 invisible"}`}>
    <span className={`${error ? "opacity-100  visible" : " opacity-0 invisible"} w-full p-2 lg:p-3 border border-red-600 bg-red-600/5 flex item-center justify-center rounded-md text-red-600  font-medium capitalize`}>{error}</span>
        <div className="flex flex-col lg:flex-row flex-wrap gap-5 py-4 w-full">
            ...
            <input  className="shadow-md rounded-md p-2 lg:w-[20%] focus:outline-none focus:border-cyan-500 focus:border" type="tel" value={employee.phone} name="phone" placeholder="123-45-678"  onChange={(e:React.ChangeEvent)=>handleChange(e)} required />
            <input  className="shadow-md rounded-md p-2 lg:w-[20%]  focus:outline-none focus:border-cyan-500 focus:border" type="date" value={employee.birthday}  name="birthday" placeholder="MM/DD/YYYY" max="2022-12-31" min="1950-01-02" onChange={(e:React.ChangeEvent)=>handleChange(e)} required />
        ...
        <span className="w-36 flex justify-between">
            <button type="submit" className="bg-cyan-600 rounded-md px-3 py-1 text-white font-medium transition-all delay-75 duration-300 ease-in-out hover:scale-110">Save</button>
